...
if __name__ == '__main__':
    ...
    for k,v in solver.net.params.iteritems():
        try:
            ...
            continue
        if ('BN' in k) or ('bn' in k) or ('loss' in k) or ('accuracy' in k):
        ...
        if 'LSUV' in init_mode:
            ...
            while (abs(needed_variance - var1) > margin):
              if ('nl' in k):
                weightsB=solver.net.params[k][0].data[:]
                weightsC=solver.net.params[k][1].data[:]
                print("Weights C:")
                print(weightsC)
#                solver.net.params[k][0].data[:] = weightsB / sqrt(var1);
                solver.net.params[k][1].data[:] = weightsC / sqrt(var1);
              else:
                weights=solver.net.params[k][0].data[:]
              ...
              if iter_num > max_iter:
                  ...
                  break; 
            if ('nl' in k):
              print("Final weights b:")
              print(weightsB)
              print("Final weights c:")
              print(weightsC)
